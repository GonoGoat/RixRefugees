...
function getAssignmentsPerUser(req, res, next) {
  pool.query("select distinct on (sessions_tasks.id) sessions_tasks_id as sess_tasks_id ,assignments.id as id, availabilities.id as availabilities_id, sessions_tasks.id sessions_tasks_id, users.id as users_id, assignments.feedback, tasks.name, sessions_tasks.amountofpeople, to_char(sessions_tasks.start_date,'DD/MM/YYYY HH24:MI') as start to_char(sessions_tasks.end_date,'DD/MM/YYYY HH24:MI') as end_date, (select count(*) from assignments as assign where availabilities_id = assignments.availabilities_id) as assignedFriends from assignments join availabilities on assignments.availabilities_id = availabilities.id join users on availabilities.users_id = users.id join sessions_tasks on availabilities.sessions_tasks_id = sessions_tasks.id join tasks on sessions_tasks.tasks_id = tasks.id where users_id = $1 and sessions_tasks.end_date > now()", [req.params.id],(err,rows) =>  {
    if (err) throw err;
